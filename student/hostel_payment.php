...
<head>
 ...
 <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=0">
  ...
  <title>hostel Payment | Malaygiri hostel</title>
        <link rel="shortcut icon" href="../images/favicon.ico" />
      <!--css and js bootstrap files-->
  <link rel="stylesheet" href="../css/bootstrap.min.css"/>
  <link rel="stylesheet" href="../css/bootstrap.css"/>
    <link rel="stylesheet" href="../css/hostel_payment.css"/>
...
if (isset($_SESSION['email']) && isset($_SESSION['password'])) {
  ...
  $queryfind="SELECT * FROM stddocuments where email='$email'";
  $checkfind=mysqli_query($connect,$queryfind);
  $count=mysqli_num_rows($checkfind);
  $fetch=mysqli_fetch_assoc($checkfind);
 //$db = mysqli_connect("localhost", "root", "alkesha15", "Malaygiri_hostel");
  if (isset($_POST['upload'])) {

 if(!is_dir("../uploaddoc/". $email ."/")) {
    mkdir("../uploaddoc/".$email ."/");
}
    //$target = "../images/".basename($_FILES['fee_file']['name']);
    $hostelpay = $_FILES['fee_file']['name'];

   $sql="UPDATE stddocuments set hostelpay='$hostelpay' WHERE email='$email'";
    mysqli_query($connect, $sql);

     if (move_uploaded_file($_FILES["fee_file"]["tmp_name"], "../uploaddoc/". $email ."/". $_FILES["fee_file"]["name"])) {
      echo '<script type="text/javascript">';
       echo 'sweetAlert("Ok", "your file uoloaded successfully", "success");';
       echo '</script>';
       echo '<script type="text/javascript">';
       echo 'window.location.href="hostel_payment.php";';
       echo '</script>';
...
?>
  ...
  <br/>
  <br/>
  <form method="POST" action="hostel_payment.php" enctype="multipart/form-data">
  <div class="row">
    <div class="col-sm-8 col-sm-offset-2">
      <div class="panel panel-default">
      <div class="panel-heading">
       <h5><label>Upload hostelPayment</label></h5>
      </div>
      <div class="panel-body">
       <div class="row file-up">
        <div class="col-sm-3">
         <label>Upload hostel Payment receipt</label>
        </div>
        <div class="col-sm-5 upload">
        <input type="file" name="fee_file" class="form-control" id="fee_file" accept="application/pdf" />
...
else
{
 ...
 <div class="alert alert-success alert-dismissable" style="margin:30px;">
  <strong>!SUCCESS</strong>You Alreay uploaded Hostel Payment Receipt Successfully
 </div>
<?php
}
?>
